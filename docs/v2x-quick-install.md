...
```shell
# 这里的外部 IP 地址要确保客户端可以访问，用于后续 omega 访问
...
(  <_> )|  |_> >\  ___/ |   |  \   / /       \  >    <  
 \____/ |   __/  \___  >|___|  / \_/  \_______ \/__/\_ \ 
  ...
  OpenV2X Omega: http://100.100.100.100:2288

  username: admin
  password: dandelion
```

上述提示中包含了 omege 的访问路径，以及用户名密码。此时可以从客户端，通过 Chrome 浏览器（其它浏览器未测试）访问试用。
...
```shell
# 这里的外部 IP 地址要确保客户端可以访问，用于后续 omega 访问，是中心站点 IP，不随着边缘站点变化
export OPENV2X_EXTERNAL_IP=100.100.100.100
# 这里的 OPENV2X_ADMIN_IP 是边缘站点服务器的 IP，不同边缘站点服务器 IP 不同 
export OPENV2X_ADMIN_IP=101.101.101.101
export OPENCV2X_IS_EDGE=true
export OPENV2X_REDIS_ROOT=password
export OPENV2X_MARIADB_ROOT=password
export OPENV2X_MARIADB_DANDELION=password
export OPENV2X_EMQX_ROOT=password
...
export OPENV2X_ENABLE_DEMO_CAMERA=true
export OPENV2X_ENDPOINT_HTTP_FLV=http://<HippoCampus-GPU-服务器-IP>:10101/live
export OPENV2X_ENDPOINT_LIDAR=ws://${OPENV2X_ADMIN_IP}:8000/ws/127.0.0.1
...
上述提示中包含了中心站点 omege 的访问路径，以及用户名密码。此时可以从客户端，通过 Chrome 浏览器（其它浏览器未测试）访问试用。
